...
Choose LangSmith when you want a managed platform with strong debugging and testing capabilities for LLM applications. LangSmith organizes traces into projects and provides detailed run inspection with inputs, outputs, and token usage. For an open-source self-hostable alternative, consider [Langfuse](/docs/providers/observability/langfuse). For workspace-level analytics, consider [Opik](/docs/providers/observability/opik).
