...
Choose DeepEval when you want a self-hosted evaluation server with a broad set of LLM metrics (faithfulness, hallucination, toxicity, and more). DeepEval is open-source and runs locally, avoiding external API dependencies for scoring. For a managed cloud platform, consider [Braintrust](/docs/reference/providers/eval/braintrust). For RAG-specific metrics, consider [RAGAS](/docs/reference/providers/eval/ragas).
