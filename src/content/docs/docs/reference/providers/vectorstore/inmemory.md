...
Use the in-memory store for unit tests and development. It requires no setup, no external services, and provides deterministic behavior for test assertions. Pair it with the [in-memory embedder](/docs/reference/providers/embedding/inmemory) for a fully self-contained test setup with no network dependencies.
