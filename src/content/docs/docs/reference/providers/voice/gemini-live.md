...
Choose Gemini Live when you want native speech-to-speech with Google's multimodal Gemini models. Gemini Live handles voice activity detection server-side, simplifying client implementation. It supports function calling and bidirectional audio over WebSocket. For OpenAI's equivalent, consider [OpenAI Realtime](/docs/reference/providers/voice/openai-realtime). For AWS-native S2S, consider [Amazon Nova S2S](/docs/reference/providers/voice/nova-s2s).
