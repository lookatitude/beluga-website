...
Traditional voice agents use a three-stage pipeline:
...
The S2S approach eliminates latency from intermediate text conversion and preserves tonal and emotional cues from the speaker's voice.

## Step 1: Initialize the S2S Provider

Create an Amazon Nova S2S provider using the registry.
...
The `LatencyTarget` option controls the tradeoff between response speed and output quality:
...
S2S requires a bidirectional streaming session where you send user audio and receive model audio concurrently.
...
The session produces audio output chunks on a Go channel. Process these concurrently to deliver audio back to the user.
...
Pipe audio from the user's microphone (or any audio source) into the session.
...
For optimal performance, send audio in small frames (20ms at 16kHz = 640 bytes per frame). Smaller frames reduce latency at the cost of higher packet overhead.

## Step 5: Interruption Handling

S2S models handle interruptions naturally because they continuously monitor the input audio stream. When the user begins speaking while the model is generating a response, the model detects the interruption and adjusts its output.

This is a fundamental advantage of S2S over traditional pipelines, where the application must explicitly coordinate STT silence detection with TTS playback cancellation.
