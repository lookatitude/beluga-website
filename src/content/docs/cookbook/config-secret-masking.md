...
1. **Pattern-based detection** — Field names are checked against a set of sensitive patterns. This catches variations like "api_key", "apiKey", and "API_KEY" without needing exact matches. This is important because different config sources might use different naming conventions.

2. **Recursive masking** — Nested maps and slices are processed recursively. This ensures that sensitive fields deep in the config structure (like `llm_providers[0].api_key`) are also masked.

3. **Non-destructive masking** — The original config is never modified. A new masked structure is created, so the original config remains intact for actual use.
